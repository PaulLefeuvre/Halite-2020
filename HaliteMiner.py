...
cellDir = ['north', 'east', 'south', 'west']
actionDir = ["NORTH", "EAST", "SOUTH", "WEST"]

def positiveCheck(ship, view_size, size, board): # Scan for the direction with the most halite
# Use a distance of four most of the time for this
    ...
    startCoord = [ship.position[0] - (view_size), ship.position[1] - (view_size)] # Get the starting top left corner
    ...
    for y in range(view_size*2 + 1): # Loop through all rows
        for x in range(view_size*2 + 1): # Loop through all cells in the row
            ...
            if(newCoord[0] != ship.position[0] and newCoord[1] != ship.position[1]):
                cellScore = board.cells[tuple(newCoord)].halite // distance(newCoord, ship.position, size) # How much that specific cell scores relative to the ship

                for i in coordSectors:
                    positiveScore[i] += cellScore
    for i in range(len(positiveScore)):
        positiveScore[i] /= 10
...
# Return an array with the danger and interest values of the surrounding squares, in the order: [center, north, east, south, west]
    ...
    for i in range(0, 4): # Loop through all four directions
        currentCell = getattr(ship.cell, cellDir[i])
        ...
        for x in range(0, 4): # Loop through all the blocks surrounding the block one away...
            checkCell = getattr(currentCell, cellDir[x])
            if(checkCell.position != ship.position): # ...except for the one the ship is currently on (we already checked it)
                if(checkCell.ship != None): # If there is a ship two blocks away
                    if(ship.halite >= checkCell.ship.halite and checkCell.ship.player != me):
...
#        me.ships[0].next_action = ShipAction.CONVERT
    ...
    for ship in me.ships:
        ...
        positiveScore = positiveCheck(ship, 3, size, board) # Divided into the four corners
        ...
        if(ship.next_action == None):
            if(len(me.shipyards) == 0):
                ...
                if(bestScore[0] == 4):
                    ...
                    ship.next_action = getattr(ShipAction, actionDir[bestScore[0]])
                    nextPositions.append(getattr(ship.cell, cellDir[bestScore[0]]).position)

            else: # If not, move away to a better spot
                bestScore = [negativeScore[0], 4] # term 0 is the actual score, term 1 is the direction as the index value of the cellDir[] and actionDir arrays
                for dir in range(4): # For each movement
                    if(dir != (len(positiveScore)-1)):
                        ...
                        if(totalScore > bestScore[0]):
                            ...
                            bestScore[1] = dir
                if(dir != 4):
                    ship.next_action = getattr(ShipAction, actionDir[bestScore[1]])
                    nextPositions.append(getattr(ship.cell, cellDir[bestScore[1]]).position)
                else:
                    ship.next_action = None
                    nextPositions.append(ship.position)
